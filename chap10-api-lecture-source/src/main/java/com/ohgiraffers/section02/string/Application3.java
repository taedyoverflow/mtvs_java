...
public class Application3 {
	...
	public static void main(String[] args) {
		...
		/* 설명. 반면 StringTokenizer의 경우 공백으로 존재하는 값을 무시해버린다. */
		...
		/* 설명. nextToken()으로 토큰을 꺼내면 해당 StringTokenizer의 토큰을 재사용 하는 것이 불가능하다. */
		while(st1.hasMoreTokens()) {
			System.out.println("st1 : " + st1.nextToken());
		}
		
		/* 설명. split()은 정규표현식 이용(문자열 패턴), StringTokenizer는 구분자 문자열 이용 */
		String colorStr = "red*oranage#blue/yellow green";
		
		/* 설명. "*#/ " 이라는 문자열이 구분자로 존재하지 않아서 에러 발생함 */
//		String[] colors = colorStr.split("*#/ ");		//에러남
		...
		/* 설명. StringTokenizer의 두 번째 인자 문자열 자체는 연속된 문자열이 아닌 하나하나를 구분자로 이용하겠다는 의미이다. */
